...
public class CustomContainer extends Container{
    ...
    public CustomContainer(InventoryPlayer playerInventory, boolean localWorld, EntityPlayer playerIn)
    {
        ...
        for (int n = 0; n < eyes; ++n)
        {
            ...
            this.addSlotToContainer(new Slot(playerInventory, 40 + (2 - n), -2+n*18, - 18)
            {
                /**
                 * Returns the maximum stack size for a given slot (usually the same as getInventoryStackLimit(), but 1
                 * in the case of armor slots)
    ...
    public ItemStack transferStackInSlot(EntityPlayer playerIn, int index)
    {
        ...
        if (slot != null && slot.getHasStack())
        {
            ...
            if (index == 0)
            {
                if (!this.mergeItemStack(itemstack1, 9+eyes, 45+eyes, true))
                {
            ...
            {
                if (!this.mergeItemStack(itemstack1, 9+eyes, 45+eyes, false))
                {
                    return ItemStack.EMPTY;
                }
            }
            ...
            {
                if (!this.mergeItemStack(itemstack1, 45+eyes, 46+eyes, false))
                {
                    return ItemStack.EMPTY;
                }
            }
            else if (index >= 9+eyes && index < 36+eyes)
            {
                if (!this.mergeItemStack(itemstack1, 36+eyes, 45+eyes, false))
                {
                    return ItemStack.EMPTY;
                }
            }
            else if (index >= 36+eyes && index < 45+eyes)
            {
                if (!this.mergeItemStack(itemstack1, 9+eyes, 36+eyes, false))
                {
                    return ItemStack.EMPTY;
                }
            }
            else if (!this.mergeItemStack(itemstack1, 9+eyes, 45+eyes, false))
            {
                return ItemStack.EMPTY;
            }
