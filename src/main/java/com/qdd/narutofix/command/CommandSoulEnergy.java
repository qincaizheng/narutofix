...
public class CommandSoulEnergy extends CommandBase {
    ...
    public String getUsage(ICommandSender sender) {
        return "/soulenergy <player> <get|set|add> <current|max> [value]";
    }
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        if (data == null) {
            throw new CommandException("No soul energy data found for target player.");
        }
        ...
        if ("get".equals(action)) {
            sender.sendMessage(new TextComponentString(player.getName() + " soul " + field + " = " + getValue(data, field)));
            return;
        }
        if (args.length != 4) {
            throw new WrongUsageException(this.getUsage(sender));
    ...
    private static double getValue(ISoulEnergyData data, String field) {
        return "max".equals(field) ? data.getMax() : data.getCurrent();
    }

    private static void setValue(ISoulEnergyData data, String field, double value) {
        if ("max".equals(field)) {
            data.setMax(value);
        } else {
    ...
    private static void addValue(ISoulEnergyData data, String field, double value) {
        if ("max".equals(field)) {
            data.addMax(value);
        } else {
