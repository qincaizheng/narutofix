...
public class DojutsuEyeHandler {
    ...
    public void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (event.phase == TickEvent.Phase.START) {
            this.reattachSusanooRider(player);
            return;
        }

        // Safety: clean up orphaned susanoo_activated state
        // (e.g. entity died from chakra depletion or combat without proper cleanup)
        this.cleanupOrphanedSusanoo(player);
    ...
    /**
     * Prevents all voluntary dismount from an active, living Susanoo.
     * This reliably blocks shift-dismount (the original narutomod's approach of
     * setSneaking(false) in onLivingUpdate has a race condition with tick order).
     *
     * Deactivation is done via ProcedureSusanoo.execute() which kills the
     * entity by stored ID. The dead entity is removed next tick, auto-dismounting
     * the rider - that auto-dismount passes through because the entity is no longer alive.
     *
     *
     */
    @SubscribeEvent
    public void onEntityMount(EntityMountEvent event) {
        if (event.getWorldObj().isRemote || event.isMounting()
                || !(event.getEntityMounting() instanceof EntityPlayer)
                || !(event.getEntityBeingMounted() instanceof EntitySusanooBase)) {
            return;
        }

        EntityPlayer player = (EntityPlayer) event.getEntityMounting();
        Entity mounted = event.getEntityBeingMounted();
        if (player.getEntityData().getBoolean("susanoo_activated") && mounted.isEntityAlive()) {
            event.setCanceled(true);
        }
    }
    ...
    private void tickCapabilityEyeDurability(EntityPlayer player, ItemStack eyeStack) {
        ...
        boolean abilityActive = player.getEntityData().getBoolean("amaterasu_active")
                || player.getEntityData().getBoolean("susanoo_activated")
    ...
    private void cleanupOrphanedSusanoo(EntityPlayer player) {
        if (!player.getEntityData().getBoolean("susanoo_activated")) {
            return;
        }
        Entity riding = player.getRidingEntity();
        if (riding instanceof EntitySusanooBase && riding.isEntityAlive()) {
            return;
        }
        // susanoo_activated is true but player is not on an alive Susanoo - clean up
        player.getEntityData().removeTag("susanoo_activated");
        player.getEntityData().removeTag("susanoo_ticks");
        Entity oldEntity = player.world.getEntityByID(
    ...
    private void reattachSusanooRider(EntityPlayer player) {
        if (!player.getEntityData().getBoolean("susanoo_activated") || player.getRidingEntity() instanceof EntitySusanooBase) {
            return;
        }

        Entity summoned = player.world.getEntityByID(player.getEntityData().getInteger("summonedSusanooID"));
        if (!(summoned instanceof EntitySusanooBase) || !summoned.isEntityAlive()) {
            return;
        }
        ...
        player.startRiding(summoned, true);
    }
}